...
export default function Restaurant() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const resp = await apiGet(`${getEndpoints.restaurantsList}${buildQuery({ is_active: filterStatus === 'active', sort_by: 'id', page, page_size: pageSize, q: query || undefined })}`);
        const ok = resp?.is_success ?? resp?.isSuccess ?? true;
        if (!ok) {
          showToast(resp?.message || 'Failed to fetch restaurants', 'error');
          return;
        }
        const list = Array.isArray(resp?.result) ? resp.result : (Array.isArray(resp) ? resp : []);
        const mapped = list.map((p) => ({
          id: p.id ?? p.restaurant_id ?? p.place_id ?? p.city_id ?? Math.random(),
          name: p.name ?? '',
        ...
        showToast(e?.message || 'Failed to fetch restaurants', 'error');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const types = useMemo(() => Array.from(new Set(data.map(p => p.food_type))).filter(Boolean).sort(), [data]);

  const filtered = useMemo(() => {
    // Client-side filtering for city/type (applied to current page only)
    return data.filter((p) => {
      const matchesCity = cityFilter === 'All' || p.city === cityFilter;
      const matchesType = typeFilter === 'All' || p.food_type === typeFilter;
  ...
  const handleView = (row) => {
    const id = row?.id;
    navigate(`/restaurant/update/${id}`, { state: { restaurant: row.raw || row } });
  };
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      const resp = await apiPatch(deleteEndpoints.restaurantDelete, payload);
      const ok = resp?.is_success ?? resp?.isSuccess ?? false;
      if (!ok) {
        showToast(resp?.message || 'Failed to delete restaurant', 'error');
        return;
      }
      showToast(resp?.message || 'Restaurant deleted', 'success');
      await (async () => {
        const r = await apiGet(`${getEndpoints.restaurantsList}${buildQuery({ is_active: filterStatus === 'active', sort_by: 'id', page, page_size: pageSize, q: query || undefined })}`);
        const list = Array.isArray(r?.result) ? r.result : (Array.isArray(r) ? r : []);
        const mapped = list.map((p) => ({ id: p.id ?? p.restaurant_id ?? p.place_id ?? p.city_id ?? Math.random(), name: p.name ?? '', city: p.city ?? p.city_name ?? p.city?.name ?? '', food_type: p.food_type ?? '', price_range: p.price_range ?? '', rating: Number(p.rating ?? 0), raw: p }));
        setData(mapped);
  ...
  const doRestore = async () => {
    ...
    try {
      ...
      const ok = resp?.is_success ?? resp?.isSuccess ?? false;
      if (!ok) {
        showToast(resp?.message || 'Failed to restore restaurant', 'error');
        return;
      }
      showToast(resp?.message || 'Restaurant restored', 'success');
      await (async () => {
        const r = await apiGet(`${getEndpoints.restaurantsList}${buildQuery({ is_active: filterStatus === 'active', sort_by: 'id', page, page_size: pageSize, q: query || undefined })}`);
        const list = Array.isArray(r?.result) ? r.result : (Array.isArray(r) ? r : []);
      ...
      showToast(e?.message || 'Failed to restore restaurant', 'error');
    } finally {
      ...
      <div className="d-flex flex-column gap-3">
        <div className="d-flex align-items-end justify-content-between gap-3">
          <div>
            ...
            <p className="text-muted small mb-0">Manage restaurants across cities and types</p>
          </div>
          <div className="d-flex align-items-center gap-2">
            ...
            <Link to="/restaurant/new" className="btn btn-gradient d-inline-flex align-items-center gap-2 text-nowrap">
          ...
          <div className="col-12 col-sm-4">
            <div className="card shadow-sm">
              <div className="card-body">
                <div className="text-muted small">Total Restaurants</div>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{row.city}</td>
                  <td>
                    <span className="badge rounded-pill text-bg-light border">{row.food_type || '—'}</span>
                  </td>
                  <td>
                    <span className="badge text-bg-light border">{row.price_range || '—'}</span>
                  </td>
